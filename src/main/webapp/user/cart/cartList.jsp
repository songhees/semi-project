...
%>
	...
	<div>
		<form id="cartInfo" action="<%--주문하는 페이지로 이동--%>" method="post">
			<table class="vinc">
				<colgroup>
					<col width="1%">
					<col width="8%">
...
%>
					<tr>
						<td>
							<input type="checkbox" name="isChecked" class="check">
						</td>
						...
						<td class="amount">
							<input style="display: inline-block; width: 22px;" type="text" name="amount" id="amount-<%=index %>" value="<%=item.getQuantity() %>">
							<span>
								<a href="" onclick="addQuantity(event, <%=index %>)"><i class="bi bi-caret-up-fill"></i></a>
								<a href="" onclick="outQuantity(event, <%=index %>)"><i class="bi bi-caret-down-fill"></i></a>
							</span>
						</td>
